...
QUnit.test("insert with autogenerated key", function(assert) {
    ...
    store.insert({ a: 1 }).done(function(values, key) {
        ...
        if(!config().useJQuery) {
            resultKey = values.key;
            resultValues = values.values;
        }

        assert.equal(typeof resultKey, "string");
        assert.ok(/[0-9a-f-]{36}/.test(resultKey));
        assert.deepEqual(resultValues, expectedValues);
        ...
        assert.deepEqual(checklist, {
            ...
            onInserted_option: [expectedValues, resultKey],
            on_inserted_callback: [expectedValues, resultKey]
        });

        store.load().done(function(data) {
            assert.equal(data.length, 1);
            assert.equal(data[0].id, resultKey);
            done();
        });
...
QUnit.test("insert with user-generated key", function(assert) {
    ...
    store.insert({ id: 42 }).done(function(values, key) {
        var resultKey = config().useJQuery ? key : values.key;

        assert.equal(resultKey, 42);
        done();
    });
...
QUnit.test("update with key", function(assert) {
    ...
    store.update(123, { name: "Bob" }).done(function(key, values) {
        ...
        if(config().useJQuery) {
            ...
            assert.equal(key.key, 123);
            assert.deepEqual(key.values, expectedValues);
        }
...
QUnit.test("modification operations without key use instance as key", function(assert) {
    ...
    store.insert(insertValues).done(function(data, key) {
        ...
        if(config().useJQuery) {
            ...
            assert.ok(data.values === insertValues);
            assert.ok(data.key === insertedObj);
        }
    ...
    store.update({ keyPart1: "0", keyPart2: "0" }, { prop: 5 })
        ...
        .done(function(key, values) {
            if(config().useJQuery) {
                ...
                assert.deepEqual(key.key, { keyPart1: "0", keyPart2: "0" });
                assert.deepEqual(key.values, { prop: 5 });
            }
        });
});
